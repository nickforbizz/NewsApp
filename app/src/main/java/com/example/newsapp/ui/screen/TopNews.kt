...
@Composable
fun TopNews(navController: NavController, articles: List<TopNewsArticle>, q: MutableState<String>, newsManager: NewsManager){
    Column(modifier = Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {
//        Text(text = "Top News", fontWeight = FontWeight.SemiBold)

        SearchBar(q, newsManager)
        val searchedText = q.value
        val resList = mutableListOf<TopNewsArticle>()

        if (searchedText != ""){
            resList.addAll(newsManager.searchedNewsResponse.value.articles ?: articles)
        }else{
            resList.addAll(articles)
        }
        LazyColumn{
            items(resList.size){
                index -> TopNewsItem(
                resList[index], onNewsClick = {
